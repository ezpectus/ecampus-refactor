...
export const EventList = ({ events, onEdit }: Props) => {
  return (
    <div className="flex flex-col gap-2">
      {events.map((event) => (
        <div key={event.id} className="border-border flex items-start justify-between gap-2 rounded-lg border p-3">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <span
                className={`text-xs font-medium ${EVENT_TYPE_TEXT_COLORS[event.type as EventType] ?? 'text-blue-600'}`}
              >
                {event.type}
              </span>
