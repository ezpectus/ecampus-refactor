...
const STUDY_TIPS: Record<string, string> = {
  ...
  grade:
    'Щоб покращити оцінки: проаналізуй, де втратила найбільше балів. Попроси детальний розбір у викладача. Зосередься на слабких темах.',
  deadline:
    'Дедлайни: використовуй метод Помодоро (25 хв роботи + 5 хв перерва). Починай з найскладнішого завдання. Розбий велике завдання на підзадачі.',
  stress:
    'Стрес перед іспитом: нормальний фізіологічний стан. Спи 7-8 годин, не вчи в ніч перед іспитом. Зроби коротку прогулянку перед заліком.',
  motivation:
    'Мотивація: згадай свою ціль — навіщо ти вчишся. Нагороджуй себе за кожну виконану задачу. Знайди напарника для спільного навчання.',
  attendance:
    'Відвідуваність важлива: пропуски лекцій = прогалини в знаннях. Якщо пропустила — попроси конспект у однокурсників, переглянь матеріал самостійно.',
  ...
  course:
    'По предмету: переглянь силабус, зрозумій структуру оцінювання. Активно беріть участь на семінарах — це дає додаткові бали.',
  project:
    'Проєктна робота: почни з планування структури. Розбий на етапи: дослідження → прототип → реалізація → презентація. Не залишай на останню ніч.',
  presentation:
    'Презентація: структуруй по принципу "проблема → рішення → результат". Тренуйся перед дзеркалом. Тайминг — 1 слайд на 1-2 хвилини.',
};

const DEFAULT_RESPONSE =
  'Я можу допомогти з порадами щодо: підготовки до іспитів, покращення оцінок, тайм-менеджменту, дедлайнів, стресу, мотивації, відвідуваності, проєктних робіт та презентацій. Про що б ти хотів дізнатися?';
